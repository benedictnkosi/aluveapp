...
<input data-res-id="' . $reservationId . '" type="text" name="check_out_time" class="input-as-text  time-picker check_out_time_input" value="' . $reservation->getCheckOutTime() . '" ' . $checkOutTimeDisabled . '></span>';
        */
        ...
        $htmlString .= '<p class="top-margin-1em"> Received on: ' . $reservation->getReceivedOn()->format('Y-m-d H:i') . '</p>';
        ...
        $this->logger->debug("check if admin " . $_SESSION["Role"]);

        if($stayCount > 0 || strpos($_SESSION["Role"], "ADMIN") > -1){
