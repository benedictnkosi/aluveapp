...
function refreshReservations(){
    $("#reservations-list").load("/api/reservations/future", function () {
        $("#stayOver-list").load("/api/reservations/stayover" , function () {
            $("#checkouts-list").load("/api/reservations/checkout" , function () {
                $("#past-res-list").load("/api/reservations/past" , function () {
                    $("#pending-res-list").load("/api/reservations/pending", function () {
                        setBindings()
                    });
                });
            });
...
function changeBookingStatus(event) {
    ...
    $("body").addClass("loading");
    $.post("/api/reservations/" + data["reservation_id"] + "/update/status/" + data["new_value"], function (response) {
        $("body").removeClass("loading");
...
function blockGuest(event) {
    ...
    if (!$("#block_note_" + article.dataset.resid).val()) {
        ...
        $("body").addClass("loading");
        $.post("/api/reservation/" + res_id + "/blockguest/" + note, function (response) {
            $("body").removeClass("loading");
...
function markReservationAsCheckedInOut(event, status) {
    ...
    $.post("/api/reservations/" + reservationID + "/update/check_in_status/" + status, function (response) {
        $("body").removeClass("loading");
        if (response[0].result_code === 0) {
            refreshReservations();
            showResSuccessMessage("reservation",response[0].result_message);
...
function captureGuestPhoneNumber(event) {
    ...
    $.post("/api/guests/" + guestID + "/phone/" + phoneNumber, function (response) {
        $("body").removeClass("loading");
...
function updateCheckInOutTime(event, $field) {
    ...
    $.post("/api/reservations/" + reservationID + "/update/"+$field+"/" + checkinTime, function (response) {
        $("body").removeClass("loading");
...
function updateCheckInDate(event, checkInDate, checkOutDate) {
    ...
    $.post("/api/reservations/" + reservationID + "/update/dates/" + checkInDate +"/" + checkOutDate , function (response) {
        $("body").removeClass("loading");
        var jsonObj = response[0];
        if (jsonObj.result_code === 0) {
            refreshReservations();
...
function updateReservationRoom(event, roomId) {
    ...
    $.post("/api/reservations/"+reservationID+"/update_room/" + roomId , function (response) {
        $("body").removeClass("loading");
        var jsonObj = response[0];
        if (jsonObj.result_code === 0) {
            refreshReservations();
...
function markAsCleaned(event) {
    ...
    if ($("#div_mark_cleaned_" + id).hasClass("display-none")) {
        ...
        $.post("/api/cleaning/"+id+"/cleaner/" +employee_id, function (response) {
            $("body").removeClass("loading");
...
function addAddOn(event) {
    ...
    if ($("#div_add_on_" + id).hasClass("display-none")) {
        ...
        $.post("/api/addon/" + add_on_id + "/reservation/" + id + "/quantity/" + quantity, function (response) {
            $("body").removeClass("loading");
            if (response[0].result_code === 0) {
                refreshReservations();
                showResSuccessMessage("reservation",response[0].result_message);
...
function addGuestID(event) {
    ...
    if (!$("#guest_id_" + article.dataset.resid).val()) {
        ...
        $("body").addClass("loading");
        $.post("/api/reservation/" + id +"/idnumber/"+idNumber, function(response) {
            $("body").removeClass("loading");
...
function addPayment(event) {
    ...
    if (!$("#amount_" + article.dataset.resid).val()) {
        ...
        $("body").addClass("loading");
        $.post("/api/payment/" + id +"/amount/"+amount, function(response) {
            $("body").removeClass("loading");
...
function addNote(event) {
    ...
    if (!$("#note_" + article.dataset.resid).val()) {
        ...
        $("body").addClass("loading");
        $.post("/api/note/"+id+"/text/" +note, function(response) {
            $("body").removeClass("loading");
