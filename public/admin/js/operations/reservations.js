    ...
    success: function (data) {
      ...
      $(".open-reservation-details").click(function (event) {
        ...
        sessionStorage.setItem(
          "reservation_id",
          event.target.getAttribute("data-res-id")
        );
...
function getReservationById(reservation_id) {
  ...
  $.ajax({
    ...
    contentType: "application/json; charset=UTF-8",
    success: function (data) {
      $("#reservation-details").html(data.html);
      $("#reservation-details").removeClass("display-none");
      setBindings();
      $("body").removeClass("loading");
    },
    error: function (xhr) {
...
function changeBookingStatus(event) {
  ...
  if (className.includes("glyphicon-triangle")) {
    data["reservation_id"] = event.target.id.replace(
      "changeBookingStatus_",
      ""
    );
  } else {
  ...
  if (className.includes("glyphicon-triangle-top")) {
    ...
    let inputResId = prompt(
      "Please enter reservation id confirm the reservation",
      ""
    );
    if (inputResId != null) {
      if (inputResId.localeCompare(data["reservation_id"]) === 0) {
...
function markReservationAsCheckedInOut(event, status) {
  ...
  if (status.localeCompare("checked_out") === 0) {
    if (
      confirm(
        "Did you collect the key from the guest? Select OK if key collected from the guest"
      ) === false
    ) {
      return;
    }
  ...
  $("body").addClass("loading");
  let url =
    "/api/reservations/" + reservationID + "/update/check_in_status/" + status;
  $.getJSON(url + "?callback=?", null, function (response) {
    if (response[0].result_code === 0) {
      refreshReservations();
...
function updateCheckInOutTime() {
  ...
  let url =
    "/api/reservations/" +
    sessionStorage.getItem("reservation_id") +
    "/update_checkin_time/" +
    checkinTime +
    "/" +
    checkOutTime;
...
function addPayment(event) {
  ...
  if (!$("#amount_" + article.dataset.resid).val()) {
    ...
    $("#payment_reference_" + article.dataset.resid).removeClass(
      "display-none"
    );
  } else {
    ...
    let paymentReference = $(
      "#payment_reference_" + article.dataset.resid
    ).val();
    if (isNaN(amount)) {
      showResErrorMessage(
        reservation,
        "Please provide numbers only for payment"
      );
      return;
    }
    ...
    if (paymentChannel.localeCompare("card") === 0) {
      if (
        paymentReference.length !== 14 ||
        paymentReference.indexOf("/") !== 4 ||
        paymentReference.lastIndexOf("/") !== 7
      ) {
        showResErrorMessage(
          "reservation",
          "Yoco reference incorrect e.g 2023/01/000037"
        );
        return;
      }
    }
    ...
    if (paymentChannel.localeCompare("transfer") === 0) {
      if (paymentReference.length < 1) {
        showResErrorMessage(
          "reservation",
          "Payment reference incorrect e.g Sibusiso M"
        );
        return;
      }
    }
    ...
    if (paymentChannel.localeCompare("card") !== 0) {
      if (
        ...
        showResErrorMessage(
          "reservation",
          "Looks like you are capturing a card reference for a " +
            paymentChannel +
            " payment"
        );
        return;
    ...
    if (paymentChannel.localeCompare("cash") === 0) {
      if (paymentReference.length > 1) {
        showResErrorMessage(
          "reservation",
          "Cash payments do not have a reference"
        );
        return;
      }
...
function addDiscount(event) {
  ...
  if (!$("#discount_" + article.dataset.resid).val()) {
    ...
    if (isNaN(amount)) {
      showResErrorMessage(
        reservation,
        "Please provide numbers only for discount"
      );
      return;
    }
    isUserLoggedIn();
