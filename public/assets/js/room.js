...
function getRoomDetails() {
  let url = "/noauth/rooms/" + getUrlParameter("id");
  $.ajax({
    type: "get",
    url: url,
    crossDomain: true,
    ...
    success: function (response) {
      ...
      for (i = 0; i < iLength; i++) {
        $(".room-amenities").append(
          '<li><a href="javascript:void(0)"><span class="fa fa-bed">' +
            json[i].name +
            "</span></a></li>"
        );
      }
...
function getRoomSlide() {
  let url = "/noauth/roomslide/" + getUrlParameter("id");
  $.ajax({
    type: "get",
    url: url,
    crossDomain: true,
