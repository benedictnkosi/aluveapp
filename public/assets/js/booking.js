          ...
          $("#checkindate").on(
            ...
            function (event, picker) {
              ...
              if (totalDays < 1) {
                showResErrorMessage(
                  "reservation",
                  "Check-in and check-out date can not be the same"
                );
                $("#checkindate").daterangepicker(
                  {
                    startDate: date,
...
function getAvailableRooms(checkInDate, checkOutDate) {
  ...
  $.getJSON(url + "?callback=?", null, function (data) {
    ...
    $(".vodiapicker option").each(function () {
      ...
      if (beds.length !== 0) {
        bedArray.forEach(
          (bed) => (bedshtml += '<span class="fa fa-bed">' + bed + "</span>")
        );
      }
      ...
      if (price.localeCompare("0") === 0) {
        var item =
          ...
          room_name +
          '<div class="select_sleeps"><span>ZAR ' +
          price +
          '</span><span class="fa fa-users">' +
          sleeps +
          " Guests</span>" +
          bedshtml +
          "</div></div>" +
        ...
        if (bedshtml.length > 1) {
          var item =
            ...
            " Guests</span>" +
            bedshtml +
            '</div><button class="btn btn-style btn-secondary book mt-3 add-room-button" data-roomId="' +
            room_id +
            '" data-roomName="' +
            room_name +
            '" data-roomPrice="' +
          ...
          var item =
            ...
            bedshtml +
            "</div>" +
            '<button class="btn btn-style btn-secondary book mt-3 add-room-button" data-roomId="' +
            room_id +
            '" data-roomName="' +
            room_name +
            '" data-roomPrice="' +
...
function createReservation() {
  ...
  if (checkInDate === checkOutDate) {
    showResErrorMessage(
      "reservation",
      "Checkin date can not be the same as check-out date"
    );
    return;
  }
  ...
  $("body").addClass("loading");
  let url =
    "/noauth/reservations/create/" +
    sessionStorage.getItem("selected_rooms_array") +
    "/" +
    guestName +
    "/" +
    phoneNumber +
  ...
  $.getJSON(url + "?callback=?", null, function (data) {
    ...
    if (data[0].result_code !== 0) {
      ...
      sessionStorage.setItem(
        "reservation_id",
        JSON.stringify(data[0].reservation_id)
      );
